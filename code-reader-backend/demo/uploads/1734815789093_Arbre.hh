...
Arbre suma (const Arbre& a) const{
  Arbre b = Arbre();
  b.primer_node = new node_arbre;
  b.primer_node = sum_aux(primer_node, a.primer_node);
  return b;
}
private:
  
static node_arbre* sum_aux(node_arbre* n,  node_arbre *m){
  node_arbre *parb = new node_arbre;
  if(n != nullptr and m != nullptr){
    parb->info = n->info + m->info;
    parb->segE = sum_aux(n->segE, m->segE);
    parb->segD = sum_aux(n->segD, m->segD);
  } else parb = nullptr;
  return parb;
}  
